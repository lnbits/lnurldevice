...
async def api_lnurldevice_create(data: CreateLnurldevice):
    return await create_lnurldevice(data)


@lnurldevice_ext.put("/api/v1/lnurlpos/{lnurldevice_id}", dependencies=[Depends(require_admin_key)])
async def api_lnurldevice_create_or_update(data: CreateLnurldevice, lnurldevice_id: str):
    return await update_lnurldevice(lnurldevice_id, data)
...
async def api_lnurldevices_retrieve(wallet: WalletTypeInfo = Depends(get_key_type)):
    ...
    return await get_lnurldevices(user.wallet_ids)


@lnurldevice_ext.get(
    "/api/v1/lnurlpos/{lnurldevice_id}", dependencies=[Depends(get_key_type)]
)
async def api_lnurldevice_retrieve(lnurldevice_id: str = Query(None)):
